error id: file:///C:/Users/maxim/43602DTU/Chisel/src/main/scala/Driver.scala:is.
file:///C:/Users/maxim/43602DTU/Chisel/src/main/scala/Driver.scala
...
non-local guesses:
	 ...
	 -scala/Predef.is().
offset: 4735
uri: file:///C:/Users/maxim/43602DTU/Chisel/src/main/scala/Driver.scala
text:
```scala
import chisel3._
...
class Driver(Kp: Double, Ki: Double, Kd: Double, PWM_frequency: Int = 30000, manual_speed_step_ms : Int = 2) extends Module {
  ...
  val calc_pos_cm = (current_position_fixed_point >> pidDP).asSInt
  val current_position_cm = Mux(system_active && calc_pos_cm > 0.S, calc_pos_cm, 0.S)
  ...
  val manual_cmd_arrived = WireDefault(false.B)
  ...
  // PID and PWM out
  val target_turns = (target_position_cm * 15.S) / 2.S
  val tolerance = 2.S
  ...
  115 is forward, 25 is backwards, 5 and 8 are the bitshifts
  */
  val pid_offset = RegNext(
    Mux(raw_pid_out > 2.S, (raw_pid_out >> 5) + 115.S,    // Forward Boost
    Mux(raw_pid_out < -2.S, (raw_pid_out >> 8) - 25.S,    // Backward Boost
    0.S))                                                 // Deadzone
  )
  ...
  val effective_control_mode = control_mode && !manual_cmd_arrived
  val active_duty = Mux(effective_control_mode, pid_duty, manual_ramped)
...
object GenerateAllVerilog extends App {
  ...
  val matlab_Kd = 0.1
  val Ts = 0.001
  emitVerilog(new Driver(matlab_Kp, matlab_Ki * Ts, matlab_Kd / Ts, 30000, 1), Array("-td", "Verilog"))
}
```
