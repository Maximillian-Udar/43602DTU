error id: file:///C:/Users/maxim/43602DTU/Chisel/src/main/scala/MotorDriver.scala:scala/Predef.`???`().
file:///C:/Users/maxim/43602DTU/Chisel/src/main/scala/MotorDriver.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/rotation_counter.
	 -chisel3/util/rotation_counter.
	 -rotation_counter.
	 -scala/Predef.rotation_counter.
offset: 3060
...
class MotorDriver(KP: Double, KI: Double, KD: Double) extends Module {
  ...
  val debounce_A       = Module(new Debouncer(5000))
  val debounce_B       = Module(new Debouncer(5000))

  debounce_A.io.btn_in := io.photo_sensor_A
  debounce_B.io.btn_in := io.photo_sensor_B

  // Registers
  val target_pos_cm = RegInit(0.S(32.W)) 
  val manual_speed  = RegInit(512.U(10.W))
  val control_mode  = RegInit(true.B)
  val manual_brake  = RegInit(true.B) // Startup Safety Lock: ON
  ...
  // --- Encoder Math ---
  rotation_counter.io.signal_A := debounce_A.io.state
  rotation_counter.io.signal_B := debounce_B.io.state
  ...
  when(txTimer >= 5000000.U) {
    switch(txState) {
      is(sTxHigh) {
        when(!tx.io.busy) {
          tx.io.data := Turns//current_pos_cm(15, 8).asUInt
          tx.io.start := true.B
          txState := sTxLow
        }
      }
